...
void create_constrained_delaunay(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F) {
    ...
    using Triangulation_type = typename std::remove_const<typename std::remove_reference<decltype(triangulation
    )>::type>::type;
...
void draw_menu() {
    ...
    if (display != DISPLAY_INPUT) {
        ...
        if (ImGui::Button("Smooth on GPU")) {
            ...
            VertexProcessor::smooth_tets_naive(TV_gpu.data(), mesh->TV.rows(), edges_gpu.data(), edges.rows(),
                                               prefix_sum_gpu.data());
